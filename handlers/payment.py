...
# ----- КЛАВИАТУРА ДЛЯ ВЫБОРА ПЛАТФОРМЫ (reply) -----
...
async def profile_callback(callback: types.CallbackQuery):
    ...
    reg_date = datetime.fromisoformat(user['registered_at']).strftime("%d.%m.%Y %H:%M") if user else "неизвестно"
    if user_id in ADMIN_IDS:
        text = (
...
async def templates_menu_callback(callback: types.CallbackQuery):
    ...
    builder.button(text="📋 Список шаблонов", callback_data="list_templates")
    builder.button(text="◀️ Назад", callback_data="main_menu")
    await callback.message.edit_text(
